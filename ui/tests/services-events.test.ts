...
it('covers registration, favorites, and account export routes', async () => {
  ...
  expect(apiMock.delete).toHaveBeenLastCalledWith('/api/me', {
    data: { [ACCESS_CODE_FIELD]: ACCOUNT_CONFIRMATION },
  });
});
...
it('covers recommendation, organizer, and participant routes', async () => {
  ...
  expect(apiMock.put).toHaveBeenLastCalledWith(
    '/api/organizer/events/17/participants/33?attended=true',
  );
});
...
it('covers student profile, personalization, and notification routes', async () => {
  ...
  apiMock.get.mockResolvedValueOnce({
    data: { email_digest_enabled: true, email_filling_fast_enabled: false },
  });
  expect((await eventService.getNotificationPreferences()).email_digest_enabled).toBe(true);

  apiMock.put.mockResolvedValueOnce({
    data: { email_digest_enabled: false, email_filling_fast_enabled: true },
  });
  expect(
    (await eventService.updateNotificationPreferences({ email_digest_enabled: false }))
